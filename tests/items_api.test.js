...
describe("When some cabinets, drawers, and users already exist", () => {
  describe("Adding a new item", () => {
    test.concurrent(
      ...
      async () => {
        ...
        const drawer = await helper.getRandomDrawerFromCabinets(
          originalCabinets
        );
        const newItem = {
          name: uuid(),
    ...
    test.concurrent(
      "without a drawer fails with 400 and returns an error",
      async () => {
        const token = await helper.getRandomAdminTokenFrom(originalUsers);
        const newItem = {
          name: uuid(),
    ...
    test.concurrent(
      "with a drawer that doesn't exist fails with 404 and returns an error",
      async () => {
        const token = await helper.getRandomAdminTokenFrom(originalUsers);
    ...
    test.concurrent(
      "with a drawer id which doesn't make sense fails with 404 and returns an error",
      async () => {
        const token = await helper.getRandomAdminTokenFrom(originalUsers);
  ...
  describe("When deleting an item", () => {
    test.concurrent(
      "Succeeds with 200, deletes item and removes item from drawer",
      async () => {
  ...
  describe("changing an item's", () => {
    test.concurrent(
      "name succeeds with 200 and returns updated drawer",
      async () => {
        const item = await helper.setupAndGetItemForTest();
        const token = await helper.getRandomAdminTokenFrom(originalUsers);
    ...
    test.concurrent(
      "drawer succeeds with 200, updates the relevant drawers, and returns updated drawer",
      async () => {
        ...
        const toDrawer = await helper.getRandomDrawerFromCabinets([
          cabinets[1],
        ]);
    ...
    test.concurrent(
      "drawer with an empty value fails with 404 and doesn't update anything",
      async () => {
        const item = await helper.setupAndGetItemForTest();
        const token = await helper.getRandomAdminTokenFrom(originalUsers);
    ...
    test.concurrent(
      "drawer with an nonexisting value fails with 404 and doesn't update anything",
      async () => {
        const item = await helper.setupAndGetItemForTest();
        const token = await helper.getRandomAdminTokenFrom(originalUsers);
  ...
  describe("getting items", () => {
    ...
    test.concurrent(
      "with an admin token succeeds with 200 and list of items",
      async () => {
        const cabinets = helper.generateRandomCabinetData(5, 1);
        await Promise.all(cabinets.map((c) => helper.populateCabinet(c)));
    ...
    test.concurrent(
      "with a non admin token succeeds with 200 and list of items",
      async () => {
        const cabinets = helper.generateRandomCabinetData(5, 1);
        await Promise.all(cabinets.map((c) => helper.populateCabinet(c)));
